...
#sys.stdout = open('file.log', "a",encoding='utf8')
"""
dictConfig({
    'version': 1,
    'formatters': {'default': {
...
def HandDetectByPhoto():
    try:
        ...
        # str(app.config['UPLOAD_Excel']) + 
        path = str(filename)+str(".jpg")
        print("------------------------------- /HandDetectByPhoto Service has been requested --------------------------------------" + str(date_time.now()) + " " + "File Name: " + str(path) + " request status " +  str(response.status_code) + "  response: " +  str(response))
